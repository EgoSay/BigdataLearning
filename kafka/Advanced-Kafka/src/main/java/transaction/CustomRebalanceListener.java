...
public class CustomRebalanceListener implements ConsumerRebalanceListener {
    ...
    public void onPartitionsAssigned(Collection<TopicPartition> partitions) {
        ...
        partitions.forEach(partition-> {
            log.warn("读取偏移量为:" + offsetManagerWithFileStore.readOffsetFromExternalStore(partition.topic(), partition.partition()));
            consumer.seek(partition, offsetManagerWithFileStore.readOffsetFromExternalStore(partition.topic(), partition.partition()));
        });
    }
}
